...
UINT getL2CacheSz() {
	...
	while (1) {
		...
		if (((cd.eax >> 5) & 0x07) == 2) {
			...
			sets = cd.ecx + 1;
			lineSz = (cd.ebx & 0x0fff) + 1;
			return ways * partitions*sets*lineSz;
		}
		i++;
	}
	return 0;
...
UINT64 readMSR(int cpu, int addr) {
	DWORD high, low;
	DWORD_PTR oldAffinity = SetThreadAffinityMask(GetCurrentThread(), 1ULL << cpu);
	Rdmsr(addr, &low, &high);
...
//
//UINT rand(UINT &r) {
//  //return r = r * 16807 % 2147483647;    // 31 bits
//  //return r = r * 48271 % 2147483647;    // 31 bits
//  return r = r * 279470273 % 4294967291;  // almost 32 bits
//}
